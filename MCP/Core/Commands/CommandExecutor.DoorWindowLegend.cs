...
namespace RevitMCP.Core
{
    public partial class CommandExecutor
    {
        ...
        private class DoorWindowLegendGroupingDiagnosticFailure
        {
            public string FailureDefinitionId { get; set; }
            public string Severity { get; set; }
            public string Description { get; set; }
            public List<IdType> FailingElementIds { get; set; } = new List<IdType>();
        }

        private class DoorWindowLegendGroupingDiagnosticSnapshot
        {
            public string Phase { get; set; }
            public string TransactionStatus { get; set; }
            public IdType ViewId { get; set; }
            public IdType GroupId { get; set; }
            public IdType GroupTypeId { get; set; }
            public string GroupName { get; set; }
            public string GroupCategory { get; set; }
            public bool GroupExists { get; set; }
            public bool GroupMetadataExists { get; set; }
            public bool GroupTypeMetadataExists { get; set; }
            public List<IdType> GroupMemberIds { get; set; } = new List<IdType>();
            public List<object> Members { get; set; } = new List<object>();
            public List<object> Dimensions { get; set; } = new List<object>();
        }

        private class DoorWindowLegendGroupingDiagnosticCaseResult
        {
            public string CaseName { get; set; }
            public string CompletionMode { get; set; }
            public IdType ViewId { get; set; }
            public Dictionary<IdType, string> MemberRoles { get; set; } = new Dictionary<IdType, string>();
            public List<IdType> MemberIds { get; set; } = new List<IdType>();
            public IdType GroupId { get; set; }
            public IdType GroupTypeId { get; set; }
            public List<string> IncludedRoles { get; set; } = new List<string>();
            public List<DoorWindowLegendGroupingDiagnosticSnapshot> Snapshots { get; set; } = new List<DoorWindowLegendGroupingDiagnosticSnapshot>();
            public List<DoorWindowLegendGroupingDiagnosticFailure> Failures { get; set; } = new List<DoorWindowLegendGroupingDiagnosticFailure>();
            public string Exception { get; set; }
        }

        private class DoorWindowLegendGroupingDiagnosticPending
        {
            public string Token { get; set; }
            public List<DoorWindowLegendGroupingDiagnosticCaseResult> Cases { get; set; } = new List<DoorWindowLegendGroupingDiagnosticCaseResult>();
        }

        private class DoorWindowLegendGroupingFailureCollector : IFailuresPreprocessor
        {
            private readonly List<DoorWindowLegendGroupingDiagnosticFailure> _failures;

            public DoorWindowLegendGroupingFailureCollector(List<DoorWindowLegendGroupingDiagnosticFailure> failures)
            {
                _failures = failures;
            }
            ...
            public FailureProcessingResult PreprocessFailures(FailuresAccessor failuresAccessor)
            {
                foreach (FailureMessageAccessor failure in failuresAccessor.GetFailureMessages())
                {
                    ...
                    _failures.Add(new DoorWindowLegendGroupingDiagnosticFailure
                    {
                        FailureDefinitionId = definitionId == null ? null : definitionId.Guid.ToString("D"),
        ...
        private class DoorWindowLegendGroupingDiagnosticCase
        {
            public string Name { get; set; }
            public string CompletionMode { get; set; }
            public string MetadataMode { get; set; }
            public HashSet<string> DimensionRoles { get; set; }
            public bool OnlyDimensionsWithInternalReferences { get; set; }
        }

        private static readonly Dictionary<string, DoorWindowLegendGroupingDiagnosticPending> DoorWindowLegendGroupingDiagnostics =
            new Dictionary<string, DoorWindowLegendGroupingDiagnosticPending>(StringComparer.OrdinalIgnoreCase);
        ...
        private object DoorWindowLegendTools(JObject parameters)
        {
        	...
        	string text2 = parameters?["mode"]?.Value<string>()?.Trim().ToLowerInvariant();
	        	if (text2 != "list" && text2 != "create" && text2 != "update" && text2 != "migrate" && text2 != "diagnose_grouping")
        	{
	        		throw new Exception("mode must be list, create, update, migrate, or diagnose_grouping");
        	}
        	...
        	int? dimensionTypeId = parameters?["dimensionTypeId"]?.Value<int?>();
	        	if (text2 == "diagnose_grouping")
	        	{
	        		string diagnosticAction = parameters?["diagnosticAction"]?.Value<string>()?.Trim().ToLowerInvariant() ?? "run";
	        		string diagnosticToken = parameters?["diagnosticToken"]?.Value<string>()?.Trim();
	        		return DiagnoseDoorWindowLegendGrouping(document, text, legendViewId, diagnosticAction, diagnosticToken);
	        	}
	        	if (text2 == "migrate")
	        	{
